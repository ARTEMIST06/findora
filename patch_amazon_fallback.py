...
# Using regex to find the amazon block
pattern = re.compile(r"if \(merchantId === 'amazon'\) \{[\s\S]*?catch \(e: any\) \{[\s\S]*?return res\.status\(500\)\.json\([^)]+\);[\s\S]*?\}[\s\S]*?\}")
match = pattern.search(code)

if match:
    newBlock = """if (merchantId === 'amazon') {
    ...
    code = code[:match.start()] + newBlock + code[match.end():]
    with open('server.ts', 'w') as f:
        f.write(code)
    print("Patched Amazon block successfully")
else:
